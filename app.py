...
def upload_file():
    ...
    try:
        ...
        tts_path = os.path.join(app.config['frontend_FOLDER'], tts_filename)
        ...
        return jsonify({
            ...
            "audio_url": f"/frontend/{tts_filename}"
        })
    ...
    try:
        ...
        if text_to_speech(translated_text, target_lang, tts_path, gender=gender):
            return jsonify({
                "success": True,
                "translated_text": translated_text,
                "audio_url": f"/frontend/{tts_filename}?t={int(os.path.getmtime(tts_path))}"
            })
        else:
            return jsonify({
                "success": True,
...
def frontend_file(filename):
    ...
    return send_file(os.path.join(app.config['frontend_FOLDER'], filename))
        ...
        for start in range(0, total_ms, chunk_ms):
            ...
            if text:
                ...
                results.append({
                    ...
                    "audio": f"/frontend/{tts_filename}" if audio_ok else None
                })
